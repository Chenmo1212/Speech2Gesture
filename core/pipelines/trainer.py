...
class Trainer(object):
    ...
    def setup_dataset(self, cfg, split, demo_input=None):
        ...
        if split == 'train':
            self.train_dataset = get_dataset(cfg.DATASET.NAME)(self.cfg.DATASET.ROOT_DIR, self.cfg.DATASET.SPEAKER,
                                                               'train', self.cfg)
            ...
            if self.cfg.TRAIN.VALIDATE:
                self.test_dataset = get_dataset(cfg.DATASET.NAME)(self.cfg.DATASET.ROOT_DIR, self.cfg.DATASET.SPEAKER,
                                                                  'val', self.cfg)
                if self.cfg.SYS.DISTRIBUTED:
            ...
            self.test_dataset = get_dataset(cfg.DATASET.NAME)(self.cfg.DATASET.ROOT_DIR, self.cfg.DATASET.SPEAKER,
                                                              'demo', self.cfg, demo_input=demo_input)
            if self.cfg.SYS.DISTRIBUTED:
                self.test_sampler = torch.utils.data.distributed.DistributedSampler(self.test_dataset, shuffle=False)
            else:
                self.test_sampler = None
    ...
    def mutiply_batch(self, batch, multiple):
        if isinstance(batch, dict):
            ...
            return batch.unsqueeze(0).repeat_interleave(multiple, dim=0).reshape(multiple * batch.shape[0],
                                                                                 *batch.shape[1:])
        else:
            raise NotImplementedError
    ...
    def train(self, cfg, exp_tag, resume_from):
        ...
        while epoch < cfg.TRAIN.NUM_EPOCHS:
            ...
            epoch_toc_mean = sum(epoch_toc_list) / len(epoch_toc_list) if len(epoch_toc_list) < 10 else sum(
                epoch_toc_list[-10:]) / 10
            ...
            for t_step, batch in enumerate(self.test_dataloader):
                if self.cfg.DEMO.MULTIPLE > 1:
                    for i in range(self.cfg.DEMO.MULTIPLE):
                        self.demo_step(batch, t_step + 1, epoch=0, extra_id=i,
                                       interpolation_coeff=i / (self.cfg.DEMO.MULTIPLE - 1))
                else:
